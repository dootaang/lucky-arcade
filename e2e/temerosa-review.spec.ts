...
test("reviews a Temerosa expression and restores the decision", async ({ page }) => {
  ...
  await expect(page.locator('.temerosa-review-scenes i[data-status="approved"]')).toHaveCount(12);
  await page.locator(".temerosa-review-actions .hold").click();
  await expect(page.locator('.temerosa-review-scenes i[data-status="approved"]')).toHaveCount(11);
