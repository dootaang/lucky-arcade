...
import type { OldMaidAction, OldMaidCard, OldMaidCartridge, OldMaidDiscard, OldMaidSeatId, OldMaidState } from "./contracts.ts";
import { OLD_MAID_VERSION, TEMEROSA_OLD_MAID_PACK_VERSION } from "./contracts.ts";

const SEAT_ORDER: readonly OldMaidSeatId[] = ["player", "pale", "kano", "nemo"];
...
export function createOldMaidState(cartridge: OldMaidCartridge, seed: string, sessionId = `old-maid-${Date.now().toString(36)}`): OldMaidState {
  ...
  shuffled.forEach((cardId, index) => hands[SEAT_ORDER[index % SEAT_ORDER.length] as OldMaidSeatId].push(cardId));
  const discards: OldMaidDiscard[] = [];
  for (const seatId of SEAT_ORDER) discardAllPairs(hands, seatId, cartridge.cards, discards, 0);
  const safeOrder = SEAT_ORDER.filter((seatId) => hands[seatId].length === 0);
  const active = SEAT_ORDER.filter((seatId) => hands[seatId].length > 0);
  const complete = active.length <= 1;
  return {
    contract: "old-maid-state/0.1", version: OLD_MAID_VERSION, packVersion: TEMEROSA_OLD_MAID_PACK_VERSION,
    sessionId, seed, sequence: 0, turn: 0, status: complete ? "complete" : "ready",
    currentPlayerId: active[0] ?? "player", hands, safeOrder, loserId: complete ? active[0] ?? null : null,
    discards, lastDraw: null,
  };
}
...
export function reduceOldMaid(cartridge: OldMaidCartridge, state: OldMaidState, action: OldMaidAction): OldMaidState {
  ...
  const index = action.type === "draw"
    ? action.index
    : cpuDrawIndex(state.seed, state.turn, actorId, targetId, targetHand.length);
  ...
  for (const seatId of SEAT_ORDER) if (hands[seatId].length === 0 && !safeOrder.includes(seatId)) safeOrder.push(seatId);
  const active = SEAT_ORDER.filter((seatId) => hands[seatId].length > 0);
  const complete = active.length <= 1;
  ...
  return {
    ...
    hands,
    safeOrder,
    loserId: complete ? active[0] ?? null : null,
    discards,
    lastDraw: { actorId, targetId, cardId, faceId: card.faceId, madePair },
  };
}

export function targetSeat(state: OldMaidState): OldMaidSeatId {
  return nextActiveSeat(state.hands, state.currentPlayerId);
}
...
export function validateCartridge(cartridge: OldMaidCartridge): void {
  assert(cartridge.contract === "old-maid-cartridge/0.1", "old_maid_cartridge_contract");
  assert(cartridge.seats.length === 4 && cartridge.seats.map((seat) => seat.id).join(",") === SEAT_ORDER.join(","), "old_maid_seats_invalid");
...
function discardAllPairs(hands: Record<OldMaidSeatId, string[]>, ownerId: OldMaidSeatId, cards: OldMaidCard[], discards: OldMaidDiscard[], turn: number): void {
  ...
  for (const cardId of hands[ownerId]) {
    const card = cardById(cards, cardId);
    if (card.pairId) byPair.set(card.pairId, [...(byPair.get(card.pairId) ?? []), cardId]);
  }
  for (const [pairId, cardIds] of byPair) if (cardIds.length === 2) {
    const pair = cardIds as [string, string];
    hands[ownerId] = hands[ownerId].filter((cardId) => !pair.includes(cardId));
    discards.push({ turn, ownerId, faceId: pairId, cardIds: pair });
  }
}
...
function nextActiveSeat(hands: Record<OldMaidSeatId, string[]>, from: OldMaidSeatId): OldMaidSeatId {
  const start = SEAT_ORDER.indexOf(from);
  for (let offset = 1; offset <= SEAT_ORDER.length; offset += 1) {
    const candidate = SEAT_ORDER[(start + offset) % SEAT_ORDER.length] as OldMaidSeatId;
    if (hands[candidate].length > 0) return candidate;
  }
  return from;
}

function emptyHands(): Record<OldMaidSeatId, string[]> { return { player: [], pale: [], kano: [], nemo: [] }; }
function cloneHands(hands: Record<OldMaidSeatId, string[]>): Record<OldMaidSeatId, string[]> { return { player: [...hands.player], pale: [...hands.pale], kano: [...hands.kano], nemo: [...hands.nemo] }; }
