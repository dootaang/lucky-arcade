...
export interface SpriteMemoryScreenProps { pack: BuiltInContentPack; title?: string; onExit(): void; }

export function SpriteMemoryScreen({ pack, title = "잔향 기억", onExit }: SpriteMemoryScreenProps) {
  ...
  return <main className="game-shell sprite-memory-shell">
    <header className="game-header"><button className="icon-button" onClick={onExit} aria-label="카지노로 돌아가기"><IconArrowLeft /></button><div><span>QUICK CABINET · {pack.title}</span><h1>{title}</h1></div><div className="game-meters"><span>{state.round}/5 라운드</span><span>{state.score}점</span></div></header>
    <section className="memory-stage">
      ...
      {state.status === "won" && <><IconBrain size={64} /><span className="eyebrow">잔향 복원 완료</span><h2>{state.score}점</h2><p>다섯 라운드의 인물 순서를 모두 정확히 복원했습니다.</p><div className="result-actions"><Button onClick={retry}><IconRefresh size={18} /> 새 순서</Button><Button onClick={onExit}>다른 게임</Button></div></>}
