...
export class LittleJsBattleAdapter implements ArcadeRuntimeAdapter<HTMLElement, BattleTranscript, { speed: 1|2|4 }> {
  ...
  #diagnostics:RuntimeDiagnostics={engine:"LittleJS 1.18.24 expedition",mountedAt:null,firstFrameAt:null,frames:0,slowFrames:0,longestFrameMs:0,destroyed:false};
  ...
  async mount(host:HTMLElement){
    ...
    if(this.#destroyed)return; const canvas=document.createElement("canvas");canvas.width=960;canvas.height=540;canvas.className="temerosa-expedition-battle-canvas";canvas.setAttribute("aria-label","피쿼드 원정 교전 재생 화면");host.replaceChildren(canvas);this.#canvas=canvas;this.#context=canvas.getContext("2d");this.#diagnostics.mountedAt=performance.now();this.#drawFrame(null,0);
  }
  ...
  #drawFrame(transcript:BattleTranscript|null,count:number){
    ...
    context.fillStyle=this.#palette.text;context.font="700 22px system-ui";context.fillText(transcript?`교전 기록 · ${transcript.tactic.toUpperCase()}`:"원정 기록을 준비합니다",28,38);
