...
export function FavoriteCupScreen({ cartridge, candidates, loadAsset, onExit }: FavoriteCupScreenProps) {
  ...
  return <main className="game-shell favorite-cup-shell">
    ...
    <section className="favorite-versus" aria-label={`${view.match[0].displayName} 대 ${view.match[1].displayName}`}>
      <Choice candidate={view.match[0]} loadAsset={loadAsset} onChoose={() => setState((current) => reduceFavoriteCup(current, view.match?.[0].npcId ?? ""))} />
      <span className="versus-mark">VS</span>
      <Choice candidate={view.match[1]} loadAsset={loadAsset} onChoose={() => setState((current) => reduceFavoriteCup(current, view.match?.[1].npcId ?? ""))} />
    </section>
  </main>;
}
...
function Choice({ candidate, loadAsset, onChoose }: { candidate: FavoriteCupCandidate; loadAsset(id: string): Promise<string>; onChoose(): void }) {
  return <button className="favorite-choice" onClick={onChoose}><Portrait candidate={candidate} loadAsset={loadAsset} featured /><strong>{candidate.displayName}</strong><small>이 인물 선택</small></button>;
}
