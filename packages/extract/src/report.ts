...
import { cardOldMaidEligibility, createCardOldMaidCartridge } from "./old-maid.ts";
...
export function createSuitabilityReport(card: ParsedCard, now = new Date()): SuitabilityReport {
  ...
  const oldMaid = cardOldMaidEligibility(createCardOldMaidCartridge(card.fingerprint, card.name, npcs.groups));
  ...
  const cabinets: CabinetAssessment[] = [
    ...
    assessment("old-maid-card", oldMaid.available, oldMaid.confidence, oldMaid.reasons),
  ...
  return suitabilityReportSchema.parse({
    contract: "suitability-report/0.3",
