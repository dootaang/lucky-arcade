...
export function CabinetRoute({ privatePreview = false }: { privatePreview?: boolean }) {
  ...
  const registration = getCabinetRegistration(cabinetId, privatePreview || status === "vip");
  const [vipAccess, setVipAccess] = useState<"checking" | "allowed" | "denied">("checking");
  useEffect(() => {
    if (status !== "vip") return;
    let alive = true;
    void import("../lib/vip.ts").then((module) => module.readVipStatus()).then((vip) => { if (alive) setVipAccess(vip.membership ? "allowed" : "denied"); }).catch(() => { if (alive) setVipAccess("denied"); });
    return () => { alive = false; };
  }, [status, cabinetId]);
  if (status === "vip" && vipAccess !== "allowed") return <main className="blocked-cabinet"><h1>{vipAccess === "checking" ? "회원권을 확인하고 있어요…" : "VIP 회원권이 필요합니다."}</h1><button onClick={() => navigate("/venues/temerosa-casino")}>카지노로 돌아가기</button></main>;
