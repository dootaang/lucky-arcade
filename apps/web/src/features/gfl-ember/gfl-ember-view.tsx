...
import { createGflRun,GFL_EMBER_VERSION,reduceGflRun,type GflAction,type GflContentPack,type GflRunState } from "@lucky-arcade/gfl-ember";
...
export default function GflEmberView({onExit}:{onExit():void}){
  ...
  useEffect(()=>{let alive=true;void loadGflContentBundle().then(async(bundle)=>{const pack=bundle.game;const recovered=await recoverSession<GflRunState,GflAction>({sessionId:SESSION,fresh:createGflRun(pack,`${new Date().toISOString().slice(0,10)}:0`,SESSION),cabinetVersion:GFL_EMBER_VERSION,packVersion:pack.version,isState:(value):value is GflRunState=>Boolean(value&&typeof value==="object"&&(value as Partial<GflRunState>).version===GFL_EMBER_VERSION&&(value as Partial<GflRunState>).packVersion===pack.version),reduce:(current,action)=>reduceGflRun(pack,current,action)});if(alive)setReady({pack,state:recovered.state});}).catch(()=>{if(alive)setError("내장 콘텐츠팩을 불러오지 못했습니다. 새로고침 후 다시 시도해 주세요.");});return()=>{alive=false}},[]);
