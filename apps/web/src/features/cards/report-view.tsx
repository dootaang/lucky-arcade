import { IconArrowRight, IconCards, IconCheck, IconHeart, IconInfoCircle, IconX } from "@tabler/icons-react";
...
export function ReportView({ card, onPlay, includePrivate = false }: { card: StoredCard; onPlay(cabinetId: string): void; includePrivate?: boolean }) {
  ...
  const oldMaid = report.cabinets.find((item) => item.cabinetId === "old-maid-card");
  ...
  return <div className="report-grid">
    ...
    {oldMaid && <CabinetCard icon={<IconCards size={32} />} eyebrow="바로 한 판" title="내 카드 도둑잡기" description="카드 속 인물이 좌석에 앉고, 표정 그림이 카드 얼굴이 됩니다." assessment={oldMaid} onPlay={() => onPlay("old-maid-card")} />}
