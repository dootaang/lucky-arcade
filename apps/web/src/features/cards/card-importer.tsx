...
import { analyzeCardFile, cancelCardAnalysis } from "../../lib/card-analysis.ts";
...
export function CardImporter({ onImported }: { onImported(card: StoredCard): void }) {
  ...
  return <section className={cn("import-zone", dragging && "is-dragging")} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop} aria-busy={busy}>
    ...
    {busy ? <><div className="hamster-loader" aria-hidden="true"><span /><span /><span /></div><div><h2>카드의 놀이 재료를 찾고 있어요</h2><p>큰 카드도 화면이 멈추지 않도록 별도 작업실에서 분석합니다.</p></div><Button onClick={cancel}>분석 취소</Button></> : <>
...
function friendlyError(reason: unknown): string {
  ...
  if (code.includes("analysis_timeout")) return "카드 분석이 안전 시간 한도를 넘었습니다. 정규식과 파일 크기를 확인해 주세요.";
