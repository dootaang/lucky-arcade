...
beforeAll(() => {
  vi.stubGlobal("fetch", async (input: string | URL | Request) => {
    ...
    const body = readFileSync(`public/content/temerosa-margin/${match[1]}/manifest.json`, "utf8");
...
describe("canonical casino side-market replay", () => {
  it("derives both game results from completed cabinet reducer transcripts", async () => {
    const base = casinoUtcSecondAtKstDay(TEMEROSA_NPC_LEDGER_CONTRACT.epochKstDay + 2, 36_000);
    ...
    const settledRaw = casinoSpectatorMarketByIdAt(TEMEROSA_NPC_GAMBLING_PROFILES, settledClock, TEMEROSA_NPC_LEDGER_CONTRACT, pairMarket!.marketId)!;
