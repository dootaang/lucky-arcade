...
새로 만들 것 (pnpm workspace — **[ROADMAP의 모듈화·무이주 계약](./ROADMAP.md) 준수, 패키지 4분할**):

- `package.json`, `pnpm-workspace.yaml`, `tsconfig.base.json` — TypeScript strict, vitest
- 의존 방향 검사(dependency-cruiser 또는 eslint import 규칙)를 `pnpm test`에 포함:
  contract ← card-io ← extract, engine 독립, DOM/React import는 전 패키지 금지
- `packages/contract/` — 공용 데이터 계약 `Sourced<T> = { value:T, source:'recovered'|'derived'|'heuristic', confidence:number(0~1), evidence:string[], derived?:string }` (derived에는 공식/근거 설명) + 카드 지문·캐비닛 계약 타입
- `packages/card-io/` — simbot `packages/card/src`에서 카드 컨테이너 파싱(charx zip 색인, PNG 카드 추출)과 카드 지문에 필요한 **순수 함수만** 이식
