...
- 무료/판돈 양쪽: 도둑잡기
- 원장 0.8에서 공개 하이로우를 2026-07-29 UTC 기준일로 편입했다. 블랙잭·다우트·원카드·텍사스 홀덤은
  계속 제외하며, 실제 개장할 때 새 원장 버전으로 편입해 과거 기록에 소급 등장하지 않게 한다.
...
## 15. LIVE PLAY TAPE 보강 (2026-07-27 오너 승인)

분 단위 정산 여섯 건만 보여 주는 기존 `최근 활동`은 화면을 매초 갱신해도 실제 새 행이 드물었다. 이를
주식 체결창처럼 읽히게 하되 잔고를 거짓으로 흔들지 않도록 **게임 중 행동**과 **실제 정산**을 분리한다.

```text
NPC 방문 세션
  → 입장·판돈·게임별 행동을 1~3초 간격의 표현 사건으로 파생
  → 세션의 기존 정산 시각에만 원장 delta 반영
```

- 표현 계약은 `npc-live-tape/0.1`이며 `(npcId, 방문 시작 UTC 초, 테이블, 2초 버킷)`의 순수 함수다.
- 표현 사건은 `NpcSession.delta`·NPC 잔고·유저 지갑·게임 `resultHash`를 읽거나 바꾸지 않는다.
- 도둑잡기·짝맞추기·슬롯·인디언 포커가 각자의 일반 행동 코드를 공급한다. 대사나 정본 사건을 생성하지 않는다.
- NPC 원장의 한 세션은 단판이 아니라 한 번의 카지노 방문 손익이다. 플로어가 비는 구간을 막기 위해 방문 표현 시간은
  테이블별 약 45~120분으로 잡되 정산 시각과 금액은 바꾸지 않는다.
- 10,000일 동안 항상 최소 4명의 NPC가 선택 가능해야 하며, 첫 365일은 플로어에 최소 한 방문이 계속 존재해야 한다.
- 화면은 최신 12행을 고정 높이로 표시하고 최근 60초 전체 행동량을 별도로 센다. 정산 행만 `P&L`로 표시한다.
- 새 행은 위에 삽입된다. 기존 행은 FLIP 이동으로 아래로 밀리고, 정산은 상승·하락 색으로 한 번 번쩍인다.
  명예의 전당은 실제 잔고가 바뀔 때만 같은 방향으로 번쩍이고 `NumberTicker`로 새 값에 착지한다.
- 모션 감축 환경에서는 흐르는 테이프·FLIP·플래시를 끄고 최신 세 건을 정적으로 제공한다. `aria-live`는 사용하지 않는다.

## 16. 방문 안의 실제 판 정산 (2026-07-27 오너 승인)

15절의 행동 테이프는 화면 연출이고 포인트를 움직이지 않는다. 반면 `최근 정산`, 명예의 전당 잔고,
테이블의 최근 결과는 `npc-live-rounds/0.1`이 만드는 **실제 세부 판 정산**을 함께 소비한다.

```text
긴 방문 한 번 (npc-ledger/0.2 NpcSession)
  ├─ 실제 판 1: 플레이어와 같은 판돈·배당표로 정산
  ├─ 실제 판 2: 플레이어와 같은 판돈·배당표로 정산
  ├─ ...
  └─ 합계: 기존 NpcSession.delta와 정확히 같음
```

- 슬롯은 약 40초, 인디언 포커는 약 90초, 짝맞추기는 약 120초마다 세부 판을 정산한다.
- 추가 판은 해당 게임의 실제 손실·당첨을 합계 0인 결정론적 묶음으로 구성한다. 방문의 마지막 판까지
  합치면 기존 `NpcSession.delta`와 정확히 같아야 한다.
- 도둑잡기는 무료 순위 보상만 사용한다. 음수나 가짜 판돈을 발명하지 않고 기존 보상을 1·3·5 P 단위의
  유효한 여러 결과로 나눈다.
- 각 세부 판은 `reservedAmount`, `creditAmount`, `delta`, `resultKind`, `termsVersion`을 보존하며
  `delta === creditAmount - reservedAmount`여야 한다.
- 방문 중에는 완료된 세부 판만 초 단위 라이브 잔고에 더한다. 방문 종료 초에는 기존 분 단위 원장이
  전체 방문 결과를 소유하고 라이브 보정을 제거한다. 이 인계에서 잔고가 튀면 실패다.
- 세부 판의 모든 접두 잔고는 `[0, 20μ]` 안이어야 한다. 세부 판 합계가 기존 방문 합계와 같으므로
  `npc-ledger/0.2`의 과거·체크포인트·일일 종가는 바뀌지 않는다.
- 최근 정산과 LIVE PLAY TAPE의 P&L 행은 같은 `NpcRoundSettlement`를 사용한다. 행동만 빠르게 흐르고
  정산이 멈춘 것처럼 보이던 두 시간축의 불일치를 허용하지 않는다.
