...
`DiscardStage`는 불투명 오버레이(`z-index:8`)이고 더미는 그 아래에 있으므로, 연속 초기 버림에서는 새 카드가
착지해도 보이지 않는다. 더미 컨테이너는 새 stacking context를 만들지 않고, **도착 중인 슬롯만**
`data-arriving="true"`와 `z-index:12`를 받아 오버레이 위로 올라온다. 착지가 끝나면 속성을 제거해 기존 더미 층으로
돌아간다. 일반 슬롯은 모두 `z-index:1`로 두고 DOM 순서로 최근 카드가 위에 놓이게 한다. 그래야 버림 수가 늘어도
뒷면 카드 열과 진행 UI(`z-index:3`)를 덮지 않는다.
