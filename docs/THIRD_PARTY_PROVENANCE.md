...
- 내장 에셋: 프로젝트 오너가 사용·가공·내장·배포 범위를 확인한 콘텐츠만 선별한다. 이후 추가하는
  모든 에셋도 같은 허가 확인 절차를 통과해야 하며, 사용자 업로드 콘텐츠의 권리와 프로그램 라이선스는
  계속 분리한다.

### 내장 콘텐츠 허가 확인

- **소녀전선: 잔불 (퇴역 기록)**: 2026-07-25 프로젝트 오너가 커뮤니티를 통해 카드·에셋의
  사용·가공·내장·배포 허가를 확보했음을 확인했다. 2026-07-30 테메로세 콘텐츠 전환으로 런타임과
  공개 배포물에서 해당 파생 에셋을 제거했으며, 이 항목은 과거 배포의 출처 기록으로만 보존한다.
- **테메로세: 여백 (`temerosa-margin`)**: 프로젝트 오너가 테메로세 시리즈 및 별도 네모 카드의
  사용·가공·내장·배포 범위를 확인했다. 공개팩은 명시적 SFW 허용 목록과 콘텐츠 감사를 통과한
  파생본만 포함한다.
- **테메로세 카지노 후보 인벤토리 (`temerosa-casino-*`)**: 위와 같은 허가 계약 아래 다섯 원본 카드의
  원본 엔트리 근거만 기록한다. A17의 Venue·슬롯·플로어 후보는 별도의 내용·SFW·시대·정본·크롭 및
  용도별 검수 전까지 `unreviewed`이며 공개팩에 포함하지 않는다. 원본 CHARX와 임시 추출물은 배포하지 않는다.
- **테메로세 제작자 직접 제공 VIP 의상·딜러 (`temerosa-vip`)**: 2026-07-29 프로젝트 오너가
  테메로세 제작자로부터 팬게임 사용을 위해 바니걸 16장과 딜러 4장(레노아 2장·이웰리스 2장)을 직접
  제공받았으며, 기존과 동일한 사용·가공·내장·배포 허가 절차를 통과했다고 확인했다. 원본 PNG 20장은
  `C:\freetalk\테메로세` 외부 소스에만 보관하고, 공개팩에는 용도별 감사와 사후 화면 확인을 거친
  WebP 파생본만 포함한다.
- **표준 트럼프 그림 카드 (`playing-cards`)**: 2026-07-25 프로젝트 오너가 원본 폴더
  `C:\freetalk\테메로세\트럼프`의 J·Q·K·Joker 렌더링본에 대해 다른 내장 에셋과 동일한
  사용·가공·내장·배포 허가 절차를 통과했음을 확인했다. 전통적인 문양이 공개적이라는 사실에
  기대지 않고, 이 특정 렌더링본에 대한 허가 확인을 근거로 13장만 파생 아틀라스에 포함한다.
